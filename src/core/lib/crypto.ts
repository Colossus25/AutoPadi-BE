...
export class Crypto {
  ...
  private static secret = appConfig.COOKIE_SECRET ?? 'secret_193a53ee38e173';
  ...
  public static encrypt(data: string, secretKey?: string): string {
    ...
    const key = crypto.pbkdf2Sync(secretKey, salt, Crypto.iterations, Crypto.keylen, Crypto.digest);
    const cipher = crypto.createCipheriv(Crypto.algorithm, key, iv);
    const enc1 = cipher.update(data, inputEncoding);
    const enc2 = cipher.final();
    const tag = cipher.getAuthTag();
    const encryptedData = Buffer.concat([iv, salt, tag, enc1, enc2]).toString(outputEncoding);
